...
class Repository(ABC, Generic[TAbstractRoot, TCriteria]):
    ...
    @abstractmethod
    def get(self, criteria: TCriteria) -> Result[TAbstractRoot, NotFoundError]: ...
    
    @abstractmethod
    def get_by_id(self, id: str) -> Result[TAbstractRoot, NotFoundError]: ...
    
    @abstractmethod
    def get_for_update(self, criteria: TCriteria) -> Result[TAbstractRoot, NotFoundError]: ...
...
class DjangoRepository(Repository[TAbstractRoot, Filters[TCriteria]]):
    ...
    def get(self, criteria: Filters[TCriteria]) -> Result[TAbstractRoot, NotFoundError]:
    ...
    def get_by_id(self, id: str) -> Result[TAbstractRoot, NotFoundError]:
        try:
            instance = self._get_queryset().get(id=id)
            instance = self._load_instance(instance)
            return Success(instance)
        except self.model.DoesNotExist:
            return Failure(NotFoundError())
    
    def get_for_update(self, criteria: Filters[TCriteria]) -> Result[TAbstractRoot, NotFoundError]:
        ...
        queryset = criteria.filter_queryset(locked_queryset)
        instance = queryset.first()
...
class InMemoryRepository(Repository[TAbstractRoot, Filters[TCriteria]]):
    ...
    def get(self, criteria: Filters[TCriteria]) -> Result[TAbstractRoot, NotFoundError]:
        ...
        instances = criteria.filter_queryset(**criteria.expressions_values)
